...
BASE_URL = os.getenv("BASE_URL", "https://purvansh01-openenv-aiops.hf.space")
...
def test_failure():
    print(f"\n{'='*50}\n--- Simulating Agent FAILING all tasks ---\n{'='*50}")
    ...
    print("\n[Client] Sending POST /step (unknown_command)")
    res = s.post(f"{BASE_URL}/step", json={"action": {"command": "unknown", "parameters": {}}})
    step1 = res.json()
    total_reward += step1.get("reward", 0.0)
    print(f"[Step 1 Reward]: {step1.get('reward')} | Cumulative: {total_reward:.2f}")
    
    print("\n[Client] Sending POST /step (resolve without fix)")
    res = s.post(f"{BASE_URL}/step", json={"action": {"command": "resolve", "parameters": {}}})
    step2 = res.json()
    total_reward += step2.get("reward", 0.0)
    print(f"[Step 2 Reward]: {step2.get('reward')} | Total Score: {total_reward:.2f}")
